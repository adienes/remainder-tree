...
bool is_power2(long long x) {
	return (x != 0) && ((x & (x - 1)) == 0);
}

long long flatten(long long i , long long j) {
	return (1<<i)+j;
}

template <typename T>
vector<T> get_subtree (const vector<T>& tree, long long k) {
...
template <typename T>
vector<T> get_shade(const vector<T>& vals, long long k) {
	...
	long long left = k; //this will be the leftmost leaf under the subtree rooted at P
	long long right = k; //similarly the rightmost leaf under subtree rooted at P
