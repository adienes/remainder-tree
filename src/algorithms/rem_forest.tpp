...
vector<T> compute_remainder_tree_layer(const vector<T>& A, const vector<U>& m, int l, T& V, U& Y, bool update_forest_vals) {
...
template <typename T, typename U>
vector<T> remainder_tree_in_forest(const vector<T>& A, const vector<U>& m, int recompute_param, T& V, U& Y, bool update_forest_vals) {
	assert (A.size() == m.size());
	...
	for (int s = 0; s < (1<<recompute_param); ++s) {
		int sub_root = flatten(recompute_param, s); //Note that sub_root is indexed in the context of the entire tree
		int C_parent = (s >> 1);
...
template <typename T, typename U>
vector<T> remainder_forest(const vector<T>& A, const vector<U>& m, int forest_param, int recompute_param, T& V, U& Y) {
	...
	for (int s = 0; s < (1<<forest_param); ++s) {
		int sub_root = flatten(forest_param, s);
		...
		for (int i = 0; i < M; ++i) { //For each leaf in the sub CTree
...
template <typename T, typename U>
vector<T> remainder_forest(const vector<T>& A, const vector<U>& m, int forest_param, int recompute_param, T V) {
	U Y;
	if (forest_param != 0) {
