...
template <typename T>
std::vector<T> subproduct_tree(const std::vector<T>&, long long P = 1);
...
std::vector<T> subproduct_tree_askew(const std::vector<T>&, const std::vector<U>&, long long P = 1);
...
template <typename T>
T compute_product_node(const std::vector<T>&, long long);

//In case we need product with a modulus
template <typename T, typename U>
T compute_product_node(const std::vector<T>&, const U&, long long);
...
template <typename T, typename U>
T compute_product_node_askew(const std::vector<T>&, const U&, long long);
...
template <typename T, typename U>
T compute_remainder_tree_value(const std::vector<T>&, const std::vector<U>&, long long, const T& V = T(1));
